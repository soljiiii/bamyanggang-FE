...
function NoticeView(props){
    ...
    useEffect(()=>{
            axios.get(`http://localhost/api/notice/noticecontent/?postNo=${prevPostNo}`)
   ...
   useEffect(()=>{
        ...
        axios.get(`localhost/api/notice/noticecontent/?postNo=${postNo}`)
        //noticelist에서 클릭한 데이터의 postNo을 저장하여, postNo가 일치한 notice데이터를 출력
        //notice 데이터를 setSelectedNotice에 저장
            .then((response) =>{
   ...
   useEffect(()=>{
    axios.get(`localhost/api/notice/noticecontent/?postNo=${nextPostNo}`)
        .then((response)=>{
